...
class HttpdRange {
	/**
		Parse a range value, return an array of all the HttpdRange
		...
		will be returned.
	**/
	static public function fromString(str : String) : Array<HttpdRange> {
		var hranges = new Array<HttpdRange>();
		...
		var parts = str.split(",");
		for(i in parts) {
			var r = new HttpdRange(i);
			if(! r.isValid()) {
				hranges = new Array<HttpdRange>();
				return hranges;
			}
			hranges.push(r);
		}
	...
	static public function satisfyAll(ranges:Array<HttpdRange>, filesize:Int) {
		for(i in ranges) {
			if(!i.isValid())
				return false;
